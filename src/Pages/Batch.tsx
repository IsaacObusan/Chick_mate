            ...
            <Card title="Batch" right={<Pill>{selectedBatch ? `Start ${formatDate(selectedBatch.startDate)}` : "Select a batch"}</Pill>}>
              ...
              <div className="grid grid-cols-1 gap-4 mt-4 sm:grid-cols-2 lg:grid-cols-4 sm:gap-8 lg:gap-12">
                ...
                <Field label="Mortality">
                  <input readOnly value={mortalityEntries.filter(m => selectedBatch && batches.find(b => b.id === batchId)?.id === batchId).reduce((sum, m) => sum + m.count, 0)} className="w-full px-4 py-2 text-sm border rounded-lg bg-gray-50" />
                </Field>
              </div>
...
{tab === 'monitoring' && (
        <div className="flex justify-end w-full px-2 m-0 mb-20 sm:pr-6 md:pr-10 lg:pr-20">
          <button className="flex items-center justify-center w-full gap-2 py-4 text-lg font-bold text-white bg-green-600 shadow-lg sm:max-w-xs hover:bg-green-700 rounded-xl" type="button">
